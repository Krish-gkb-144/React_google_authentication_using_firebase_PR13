...
const Login = () => {
  ...
  const ongoogle = async () => {
    try {
      ...
      localStorage.setItem('checkUserLogin', JSON.stringify(record.user));
      toast.success("Successfully Login");
      navigate('/');
    } catch (err) {
      return false;
      ...
      <div className='pt-5'>
        <div className='container d-flex justify-content-center'>
          <div className='col-6 bg-white rounded-4'>
            <h1 className='text-center pt-5'>User Login</h1>
            <div className='row p-5'>
              <form className='fs-5'>
                ...
                <button type="button" className="btn btn-outline-primary" onClick={ongoogle}>Sign in with Google</button>
